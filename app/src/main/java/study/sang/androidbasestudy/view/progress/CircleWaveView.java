...
public class CircleWaveView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        mPath.addCircle(centerX,centerY, radio,Path.Direction.CCW);
        canvas.clipPath(mPath);
        mPath.reset();
        for (int i = (int) rectF.left; i < rectF.right; i++) {
            canvas.drawLine(i, rectF.bottom, i, (float) (height + (mHeight-height) * Math.sin((i - rectF.left - deep) * Math.PI / 180) / 10), mPaint);
            canvas.drawLine(i, rectF.bottom, i, (float) (height + (mHeight-height) * Math.sin((i - rectF.left - deep + 180) * Math.PI / 180) / 10), mPaint);
        }
